...
describe('Lines', function() {
    ...
    it('returns true if points are collinear', function(){
        var A = V([-1,-8])
          , B = V([1,-2])
          , C = V([2,1])
          , D = V([16,20,20])
        ...
        expect(gm.collinear(A, B, C)).to.be(true)
        expect(gm.collinear(D, E, F)).to.be(false)
        expect(gm.collinear(G, H, I)).to.be(true)
    })
})
